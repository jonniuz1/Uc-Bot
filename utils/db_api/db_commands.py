...
class DB:
    ...
    def create_table_users(self):
        ...
        {REFERRAL_LINE_NAME} int ,
        {INVITES_COUNT_LINE_NAME} int NOT NULL default 0,
        {ACTIVE_LINE_NAME} INTEGER DEFAULT 1);
        '''
    ...
    def add_user(self, identifier: int, referral: str, active, invites_count=0):
        command = f'''
        INSERT INTO {TABLE_USERS_NAME} VALUES(?, ?, ?, ?)
        '''

        self.execute(command, (identifier, referral, invites_count, active), commit=True)
    ...
    def check_referral(self, ref: int) -> bool:
        command = f'''
    ...
    def update_invites_count(self, identifier: int):
        command = f'''
                UPDATE {TABLE_USERS_NAME} SET {INVITES_COUNT_LINE_NAME} = ? WHERE {IDENTIFIER_LINE_NAME} = ? 
                '''
        current = self.get_invites_count_by_id(identifier)
        self.execute(command, (current + 1, identifier,), commit=True)

    def get_invites_count_by_ref(self, ref: int) -> int:
        command = f'''
